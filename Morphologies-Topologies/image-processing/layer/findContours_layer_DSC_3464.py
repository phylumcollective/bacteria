...
img = loadImg('../img/layer/DSC_3464.JPG', gray=True)
img = scaleImg(img)
img2 = loadImg('../img/layer/DSC_3464.JPG')
img2 = scaleImg(img2)
...
for i in range(len(contours)-2):
    ...
    if hierarchy[0][i][3] == -1 and area > 90:  # include small contours (aggregate)
        M = cv2.moments(contours[i])
        # calculate x,y coordinate of centroid & draw it
        cX = int(M["m10"] / M["m00"])
        cY = int(M["m01"] / M["m00"])
        if cY < img.shape[0] - 290 and cX < img.shape[1] - 550 and (cY * cX) > 99000 and cY > 300 and cX > 450:
...
cv2.imshow("Image", scaleImg(loadImg('../img/layer/DSC_3464.JPG')))
...
while True:
    ...
    if key == 115:
        cv2.imwrite('../img/layer/DSC_3464_contours_out.jpg', out)
        cv2.imwrite('../img/layer/DSC_3464_contours_out1.jpg', out1)
        break
    if key == 27:
        break
