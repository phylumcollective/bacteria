...
img = loadImg('../img/blossom/DSC_3630.JPG', gray=True)
img = scaleImg(img)
img2 = loadImg('../img/blossom/DSC_3630.JPG')
img2 = scaleImg(img2)
...
for i in range(len(contours)-2):
    ...
    if hierarchy[0][i][3] == -1 and area > 566:
        ...
        cX = int(M["m10"] / M["m00"])
        cY = int(M["m01"] / M["m00"])
        if cX < img.shape[1] - 328 and cX > 400:
...
cv2.imshow("Image", scaleImg(loadImg('../img/blossom/DSC_3630.JPG')))
...
while True:
    ...
    if key == 115:
        cv2.imwrite('../img/blossom/DSC_3630_contours_out.jpg', out)
        cv2.imwrite('../img/blossom/DSC_3630_contours_out1.jpg', out1)
        break
    if key == 27:
        break
