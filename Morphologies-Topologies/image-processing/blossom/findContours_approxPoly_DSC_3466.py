...
img = loadImg('../img/blossom/DSC_3466.JPG', gray=True)
img = scaleImg(img)
img2 = loadImg('../img/blossom/DSC_3466.JPG')
img2 = scaleImg(img2)
...
# blur & threshold
imgBlur = cv2.medianBlur(img, 13)  # path attribute: manifold (limit blurring)
...
for i in range(len(contours)-2):
    ...
    if hierarchy[0][i][3] == -1 and area > 10:  # include small contours (aggregate)
        M = cv2.moments(contours[i])
        # calculate x,y coordinate of centroid & draw it
        cX = int(M["m10"] / M["m00"])
        cY = int(M["m01"] / M["m00"])
        if cY < img.shape[0] - 1 and cX < img.shape[1] - 350 and cY > 200 and cX > 500:
...
cv2.imshow("Image", scaleImg(loadImg('../img/blossom/DSC_3466.JPG')))
...
while True:
    ...
    if key == 115:
        cv2.imwrite('../img/blossom/DSC_3466_contours_out.jpg', out)
        cv2.imwrite('../img/blossom/DSC_3466_contours_out1.jpg', out1)
        break
    if key == 27:
        break
